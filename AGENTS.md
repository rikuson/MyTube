...
ユーザーの指示により、速度比較のためCodexによる検索語整理・動画評価を停止中。入力をyt-dlpのYouTube検索に直接渡し、1ページ25件をYouTubeの取得順で表示する。関連性による除外や重みづけは行わない。動画IDの検証・重複除去と検索結果からの再生制限は維持する。取得時間は画面に表示しない。Codexのログインや利用枠は不要。以下のCodex選別方針は元の設計であり、現行動作ではない。
...
- YouTubeアカウントとの認証・同期方法 → `yt-dlp --cookies-from-browser chrome` で実装済み。Chrome 固定。
...
- ユーザーの追加指定により、アカウントの再生リストを視聴の入口として許可する。サイドバー幅320pxを維持し、MUI Collapseで「再生リスト」「登録チャンネル」を個別に折りたたむ。見出しをチャンネル一覧の上に配置し、再生リストへのアクセスを確保する。
- 再生リストは初回展開時にChromeのcookieとyt-dlpで `https://www.youtube.com/feed/playlists` から取得する。IDを検証・重複除去し、Mix（RD系）は除外する。取得済み一覧と動画ページをメモリで保持し、ヘッダーロゴで再取得する。
- `src-tauri/src/playlists.rs` と `src/usePlaylists.ts` が担当する。取得済み再生リストのIDのみ動画取得を許可し、26件目を次ページ判定に使って25件ずつ表示する。ページ内の動画IDを検証・重複除去し、取得済み動画のみ再生と詳細取得を許可する。
- 取得は120秒でタイムアウトし、アプリ終了時にプロセスを停止する。画面切り替え後に古い応答が表示を上書きしないよう要求番号で制御する。再生リストでも次の動画へ自動再生しない。
- 実アカウントでfeedから14件の再生リスト形式のエントリを取得できることを確認した。アカウントの内容はログ・リポジトリに保存しない。

- 単一画面構成。タブは無い。
- ヘッダー: アプリ名 + 検索入力（単行 input）。検索語があれば検索結果、空なら登録チャンネルを表示。
- 登録チャンネルは起動時・検索クリア時に自動表示。ヘッダーのロゴから再同期可能。サイドバーの検索欄は取得済みの登録チャンネルを入力中に絞り込む。
- デスクトップ表示のサイドバー幅は320px。チャンネル検索欄は高さ32pxで表示する。
- 検索結果・登録チャンネル共通で VideoCard コンポーネントを使用。
- ページネーション: ホーム、検索結果、選択したチャンネルの動画を「前の25件/次の25件」で切り替える。
- 一覧と再生画面の遷移は `src/viewHistory.ts` に集約する。再生中に検索・チャンネル・再生リストへ移る場合は現在の再生履歴を一覧へ置換し、戻る操作で古い再生画面を復元しない。
- MUI（Material UI）とEmotionを使用。テーマは `src/main.tsx` に集約。

動画一覧は25件ごとにページを切り替えます。「前の25件」「次の25件」で移動できます。
...
- チャンネル選択時は同期結果の `channel_id` を使い、`https://www.youtube.com/channel/<channel_id>/videos` からページごとに26件を取得する。先頭25件を表示し、26件目の有無で次ページを判定する。ホームは同期時に取得した最大200件を保持し、25件ずつ表示する。
- 使用するブラウザは Chrome 固定。Cookie / 認証情報は yt-dlp 側で macOS Keychain から読み取る。MyTube 側で cookie を保存・ログ・リポジトリへ残さない。
- 同期は起動時および検索クリア時に自動実行し、ヘッダーのロゴから任意に再実行できる。同期中・エラー・完了は `SubscriptionsStatus` で UI に反映する。
...
- 自動再生・関連動画・おすすめフィードは一切追加しない。検索画面と同様の制約を適用する。

### 検証コマンド

- `cargo test --manifest-path src-tauri/Cargo.toml subscriptions::tests -- --nocapture`: 登録チャンネル同期の単体テスト（Chrome 固定、キャンセル、feed 解析・ID 検証・重複除外）。
